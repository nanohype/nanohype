...
K6 load tests for the __PROJECT_NAME__ HTTP service.
...
# Run with default config (localhost:3000)
...
- `create_item_duration` -- trend for POST /api/items response time
- `get_item_duration` -- trend for GET /api/items/:id response time
...
```js
group("POST /api/your-endpoint", () => {
  const res = http.post(`${BASE_URL}/api/your-endpoint`, payload, params);
  check(res, {
