...
import type { MediaProvider, MediaProviderFactory } from "../providers/types.js";
...
function stubFactory(name: string): MediaProviderFactory {
  return () => ({
    name,
    async init() {},
    async upload() {
...
describe("media provider registry", () => {
  ...
  it("lists built-in providers after barrel import", async () => {
    ...
    expect(names).toContain("uploadcare");
    expect(names).toContain("imgix");
