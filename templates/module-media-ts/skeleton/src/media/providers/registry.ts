// ── Provider Registry ──────────────────────────────────────────────
//
// Factory-based registry for media providers. Each provider module
...
import type { MediaProvider, MediaProviderFactory } from "./types.js";

const factories = new Map<string, MediaProviderFactory>();

/**
 * Register a provider factory. Called by each provider module at
 * import time (self-registration pattern).
 */
export function registerProvider(name: string, factory: MediaProviderFactory): void {
  if (factories.has(name)) {
    throw new Error(`Media provider "${name}" is already registered`);
  }
...
export function getProvider(name: string): MediaProvider {
  ...
  if (!factory) {
    ...
    throw new Error(`Media provider "${name}" not found. Available: ${available}`);
  }
  return factory();
}
