...
import type { ContentBlock, LlmProvider, LlmResponse, Message, StreamChat } from "./types.js";
...
class MockProvider implements LlmProvider {
  ...
  streamChat(_systemPrompt: string, messages: Message[], _tools: Tool[]): StreamChat {
    const text = pickResponse(messages);
    const content: ContentBlock[] = [{ type: "text", text }];
    const rawAssistantMessage: Message = { role: "assistant", content };
