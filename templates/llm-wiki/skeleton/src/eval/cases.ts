...
/** One case: a question to ask the wiki, and what must hold of the answer. */
...
export interface CaseAssertion {
  type: "contains" | "not_contains" | "matches_pattern" | "cites_page";
  value: string;
  why?: string;
}
...
function isAssertion(value: unknown): value is CaseAssertion {
  ...
  return typeof a.type === "string" && a.type.length > 0 && a.value !== undefined;
}
 ...
 * plain error naming the file when one of them is not a case — whether it
 * fails to parse or parses to the wrong shape. A malformed case that is
 * silently skipped is a case that stops running without anything saying so.
 */
  ...
  if (cases.length === 0) {
    throw new EmptyCorpusError(
      `No eval cases in ${dir}. An eval that runs nothing reports the same result as one ` +
        ...
        ".json file there holding a name, a kind of golden or adversarial, the question to ask, " +
        "and at least one assertion carrying a type and a value.",
    );
  }
