 ...
 * ```
 */
export function createProjectClient(rawConfig: Partial<ProjectConfig> = {}): ProjectClient {
  ...
  async function updateIssue(id: string, input: IssueUpdate, provider?: string): Promise<Issue> {
    const name = provider ?? config.defaultProvider;
    const p = getProvider(name);
  ...
  async function addComment(issueId: string, body: string, provider?: string): Promise<Comment> {
    const name = provider ?? config.defaultProvider;
    const p = getProvider(name);
