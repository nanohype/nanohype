// ── Provider Registry ──────────────────────────────────────────────
//
// Factory-based registry for search providers. Each provider module
...
import type { SearchProvider, SearchProviderFactory } from "./types.js";
 ...
 */
export function registerProvider(name: string, factory: SearchProviderFactory): void {
...
export function getProvider(name: string): SearchProvider {
  ...
  if (!factory) {
    ...
    throw new Error(`Search provider "${name}" not found. Available: ${available}`);
  }
  return factory();
}
