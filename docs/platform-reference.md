...
| [`nanohype/fab`](https://github.com/nanohype/fab)                               | Reference factory client. Orchestrates 80 Claude agents across Discovery → Design → Build → Verify → Ship. Four transports, selected by `FAB_RUNTIME`. Ships the quality-check rubric and per-role curation and engineering baselines in `skills/`; overlay your personal recipe via `~/.fab/skills/`          | `fab/skills/README.md`         |
...
- **Model path** — inference is Bedrock in the tenant's own account, including open-weight models via Bedrock Custom Model Import (routed by a `ModelGateway` `modelSource: imported` route referencing the imported-model ARN). Embeddings stay on Bedrock foundation embedding models — Custom Model Import does not support embedding models. In-cluster model serving is not a supported shape, and GPU nodes are not part of the model story: open-weight flexibility comes through the ordinary Bedrock runtime with no accelerators
...
| [`platform-tenant-contract.json`](../standards/platform-tenant-contract.json)   | The required artifacts (chart, ApplicationSet entry, Platform CR), the minimum Platform CR shape, OTel resource attrs, and the chart rules with their severities (no in-chart IAM role, no chart-owned ServiceAccount, no role-arn annotation)                                                                                                 |
| [`llm-policy.json`](../standards/llm-policy.json)                               | Bedrock-primary, IAM-role auth (Pod Identity on EKS), model tiers (sonnet default / opus escalation / haiku light), the deploy region, prompt-caching requirement                                                                                                                                                                              |
| [`quality-rubric-dimensions.json`](../standards/quality-rubric-dimensions.json) | The ten quality dimensions every build is graded against, and the ids every other standard's `grades` field draws from. Dimension names + summaries only — reviewer assignments and merge-gate enforcement live in the reference client                                                                                                        |
...
| [`observability-slo.json`](../standards/observability-slo.json)                 | RED for services, USE for resources, the four golden signals, and at least one SLO with a multi-window multi-burn-rate error budget. Fixes the dashboard a system ships to represent itself, plus the fleet-alerting contract — severity tiers, standard alarm dimensions, and per-cluster composite rollups so a hard-down cluster pages once |
| [`telemetry-pipeline.json`](../standards/telemetry-pipeline.json)               | How telemetry travels — the OTLP collection contract every workload emits against one collector-agnostic endpoint, the `floor`\|`full` cluster tier that changes only where signals land, the events the platform publishes when telemetry makes it act, and the SSM paths a client discovers those resources from                             |
...
| [`documentation-voice.json`](../standards/documentation-voice.json)             | How prose is written on every surface a human reads — markdown, code comments, schema descriptions, CLI help, error strings — citing the Google developer documentation style guide and OKF, plus the four rules neither covers                                                                                                                |
| [`agent-access.json`](../standards/agent-access.json)                           | The AI agents and search crawlers every public deliverable admits — the fetcher roster by token, operator and class, and the robots.txt, edge and live-probe rules that keep search-index and user-initiated fetchers reachable while a site opts out of training                                                                              |

Each file is validated against [`schemas/standards.schema.json`](../schemas/standards.schema.json), and declares `applies_to` (the deliverables it governs, with any stack scoping) and `grades` (the quality dimensions a violation is graded on); every rule carries a `reject` or `warn` severity. [`standards/README.md`](../standards/README.md) is the human-readable normative form.

**What's intentionally not in this directory**: the merge-gate choreography, the per-reviewer dimension assignments, the agent roster, the factory preamble prompt, and the orchestration code that produces consistent output against this bar. Those live in the reference client. The cut is deliberate — publishing the guardrails everyone has to meet, keeping the choreography that consistently hits them.
...
The overlay system. Fab bundles the quality-check rubric and per-role curation and engineering baselines as markdown files in `fab/skills/`. When loading a skill, fab walks four locations in priority order — `$FAB_SKILLS_DIR` → `~/.fab/skills/` → `<cwd>/.fab/skills/` → bundled — and the first match wins as the base. `<skill>.append.md` files from every layer get concatenated for additive overlays. Result: anyone can clone fab and run a competent factory; your personal overlay at `~/.fab/skills/` produces _your_ factory.
...
2. **Load the standards**: same URL pattern for `standards/*.json`. Each file's `applies_to` says whether it governs your deliverable, and its `grades` which quality dimension a violation lands on.
...
- `catalog.json`: v1
- `standards/*.json`: the `version` field in each file
- `@nanohype/sdk`: see [`sdk/package.json`](../sdk/package.json)
- `@nanohype/mcp`: see [`mcp-server/package.json`](../mcp-server/package.json)

## Reusing this pattern

The structure described here (top-level reference doc + machine-readable manifests + per-repo `AGENTS.md` + SDK + MCP server) is portable. If you want to publish a similar Platform Reference for your own org or project, [`docs/platform-reference-pattern.md`](platform-reference-pattern.md) describes how.
