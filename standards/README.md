...
`tenant-chart-base` renders the PrometheusRule (SLI recording rules + the four burn-rate alerts) and the optional ServiceMonitor from this standard's shape; every k8s tenant inherits it with observability **on by default**. The baseline pushes metrics via OTLP to the cluster collector — the standard's accepted "equivalent scrape config" — so the ServiceMonitor is opt-in for apps that expose a Prometheus `/metrics` endpoint instead.
